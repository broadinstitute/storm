"""Implementation of the ``storm`` package public API.

Imported on demand via :func:`storm.__getattr__` so ``storm/__init__.py`` can
finish without loading the native extension first. That avoids re-entrant
``import storm`` while ``storm.storm`` is initializing, which can leave
``storm`` partially built (missing ``build_tr_sidecar``, ``version``, etc.) on
some environments.
"""
...
_storm_ext = None
_storm_ext_loaded = False


def _get_storm_ext():
    global _storm_ext, _storm_ext_loaded
    if _storm_ext_loaded:
        return _storm_ext
    _storm_ext_loaded = True
    try:
        import storm.storm as ext  # type: ignore[import-not-found]
    except ImportError:
        _storm_ext = None
    else:
        _storm_ext = ext
